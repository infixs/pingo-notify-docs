...
O Pingo Notify transforma eventos (como "Novo Pedido") em mensagens de WhatsApp personalizadas.
...
### Trigger e Destino
1.  **Trigger (Gatilho):** O evento que dispara a notificação (ex: "Order Status Change" para "Concluído").
2.  **Conexão:** Qual WhatsApp da sua loja enviará a mensagem.
3.  **Destinatário:** Quem recebe. Use variáveis dinâmicas (ex: `{{ order.shipping.phone }}`) em vez de números fixos.
...
*   **Exemplo Prático**: Imagine uma regra para enviar apenas se o `order.total` for maior que **R$ 100**.
    *   Se um pedido de R$ 50 entrar, **o envio é cancelado** silenciosamente.
...
1.  Troque a **Exemplo de Dados** (no topo esquerdo) para diferentes pedidos.
...
*   **Testar:** Use o botão **Send Test** para enviar a mensagem ao seu número pessoal antes de ativar (`Active`).

---

## 3. Sintaxe do Template

Use o padrão `Handlebars` para inserir dados dinâmicos.
...
```handlebars
Olá {{ customer.first_name }}, recebemos seu pedido!
```
...
```handlebars
Olá *{{ customer.first_name }}*! Tudo bem? 👋
...
💳 Parcelado em {{ order.installments }}x.
{{ /if }}
