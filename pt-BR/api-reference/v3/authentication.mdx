...
<Warning>
É o header **`apikey`** — não `Authorization: Bearer`. Um header bearer é ignorado, e a requisição falha com `401`.
</Warning>
...
**Conexões oficiais do WhatsApp e campanhas exigem um plano pago**, e tanto conexões quanto mensagens são medidas contra os limites do seu plano. Quando um limite é atingido você recebe um **403** com um código como `USER_PLAN_EXCEEDED_CONNECTIONS` ou `USER_PLAN_EXCEEDED_MESSAGES`. Consulte sua folga a qualquer momento com `GET /v3/summary`.

## Endpoints deliberadamente públicos

Um punhado de rotas não leva `apikey`, porque quem as chama não pode ter um. Em cada caso, um token assinado na URL *é* a credencial:
...
| `GET`/`POST /v3/webhooks/whatsapp/{connectionId}` | A Meta | `x-hub-signature-256`, mais o verify token |
| `POST /webhooks/helpdesk/{inboxId}` | Seu próprio app | `x-helpdesk-token` ou `x-helpdesk-signature` |
