...
:::card-group{cols=2}
  ::card{title="Autenticação" icon="key" to="/pt-BR/api-reference/v3/authentication"}
  Seu token de API, a escolha do workspace, e o que cada papel pode fazer.
  ::

  ::card{title="Convenções" icon="list-check" to="/pt-BR/api-reference/v3/conventions"}
  Paginação, erros, ids e datas — as regras que todo endpoint segue.
  ::

  ::card{title="Webhooks" icon="bolt" to="/pt-BR/api-reference/v3/webhooks"}
  Receba mensagens e eventos no seu servidor, e verifique que vieram de nós.
  ::

  ::card{title="Tempo real" icon="tower-broadcast" to="/pt-BR/api-reference/v3/realtime"}
