...
:::card-group{cols=2}
  ::card{title="Authentication" icon="key" to="/en/api-reference/v3/authentication"}
  Your API token, choosing a workspace, and what each role can do.
  ::

  ::card{title="Conventions" icon="list-check" to="/en/api-reference/v3/conventions"}
  Pagination, errors, ids and dates — the rules every endpoint follows.
  ::

  ::card{title="Webhooks" icon="bolt" to="/en/api-reference/v3/webhooks"}
  Receive messages and events on your own server, and verify they came from us.
  ::

  ::card{title="Realtime" icon="tower-broadcast" to="/en/api-reference/v3/realtime"}
