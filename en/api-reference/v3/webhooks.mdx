...
- **Connection webhooks** deliver raw WhatsApp traffic — a message arrived, a message was delivered, the connection dropped. This is what you want to drive your own logic on top of WhatsApp.
...
Leave `connections` out to receive from every connection in the workspace, or list connection ids to narrow it.
...
| `send.message` | A message **went out** — through the API, or typed on the phone. |
...
<Tabs>
  ...
  <Tab title="send.message">
    A message **sent from your connection** — via the API, or typed on the phone. `data` has exactly the same shape as `messages.upsert`, with two differences that matter:
...
Answer with any **2xx** and the delivery is done. Answer **4xx** or **5xx**, or time out, and Pingo retries — **3 attempts total**, backing off exponentially from 5 seconds. The request times out after 10 seconds.
...
Create an inbox with `channelType: "API"` and keep the `inboundWebhookSecret` it returns — it is shown only once:
